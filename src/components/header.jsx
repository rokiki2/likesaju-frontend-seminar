...
export const Header = () => {
  ...
  const linkStyle =
    'text-xl font-bold text-[#14142B] leading-6 hover:font-extrabold hover:text-[#4A3AFF] hover:cursor-pointer mobile:text-base mobile:leading-5';
  const activeLinkStyle = 'text-xl font-extrabold text-[#4A3AFF] leading-6 mobile:text-base mobile:leading-5';
  ...
  return (
    <div className="sticky top-0 w-full flex flex-row items-center justify-between bg-white drop-shadow h-[80px] px-[68px] z-[999] mobile:h-[60px] mobile:px-4">
      <Link
        to="/"
        className="text-[26px] font-extrabold text-[#14142B] leading-9 tracking-tighter mobile:text-lg mobile:leading-7"
      >
        멋쟁이 사주처럼
      </Link>
      <div className="flex flex-row items-center gap-[50px] mobile:gap-[20px]">
        ...
        {isLogin ? (
          ...
          <Link
            to="login"
            className="text-xl font-bold text-[#4A3AFF] leading-6 bg-[#F3F1FF] px-7 py-[17px] rounded-[50px] mobile:text-base mobile:leading-5 mobile:px-4 mobile:py-2"
          >
            로그인
          </Link>
        )}
